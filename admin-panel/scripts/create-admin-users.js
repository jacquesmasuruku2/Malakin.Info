...
async function ensureAdminSchema() {
  ...
  const defaultHash = bcrypt.hashSync('678900', 10);
  const adminEmail = 'jacquesmasuruku2@gmail.com';
  const defaultName = 'Jacques Masuruku';

  await client.query(
    `INSERT INTO public."AdminUser" ("id", "email", "name", "passwordHash", "provider", "role", "isActive", "emailVerified", "createdAt", "updatedAt")
     VALUES (gen_random_uuid()::STRING, $1, $2, $3, 'email', 'admin', true, true, now(), now())
     ON CONFLICT ("email") DO UPDATE SET
       ...
       "updatedAt" = now();`,
    [adminEmail, defaultName, defaultHash],
  );
