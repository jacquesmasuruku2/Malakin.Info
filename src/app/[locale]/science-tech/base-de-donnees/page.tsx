import Link from 'next/link';
import ArticleListingGrid from '@/components/ArticleListingGrid';

export default async function BaseDeDonneesPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;

  const items = [
    {
      id: 1,
      title: 'Introduction aux bases de données relationnelles',
      date: '27 Juin 2026',
    },
    {
      id: 2,
      title: 'MongoDB vs PostgreSQL : Lequel choisir ?',
      date: '24 Juin 2026',
    },
    {
      id: 3,
      title: 'Optimisation des requêtes SQL',
      date: '23 Juin 2026',
    },
    {
      id: 4,
      title: 'NoSQL : Quand et pourquoi l\'utiliser ?',
      date: '22 Juin 2026',
    },
  ];

  const articles = items.map((item) => ({
    id: String(item.id),
    slug: String(item.id),
    title: item.title,
    dateLabel: item.date,
    categoryTitle: 'Base de Données',
  }));

  return (
    <div className="tag-page bg-background">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-16 sm:pt-14">
        <Link
          href={`/${locale}/science-tech`}
          className="mb-8 inline-block text-sm text-muted-foreground hover:text-foreground"
        >
          ← Retour à Science & Tech
        </Link>
        <h1 className="font-heading text-3xl sm:text-4xl font-bold text-foreground tracking-tight mb-3 sm:mb-4">
          Base de Données
        </h1>
        <p className="text-base sm:text-lg text-muted-foreground mb-8 sm:mb-10 max-w-3xl">
          Tutoriels, guides et ressources sur les systèmes de gestion de bases de données
        </p>

        <ArticleListingGrid
          articles={articles}
          locale={locale}
          hrefFor={(item) => `/${locale}/science-tech/base-de-donnees/${item.slug}`}
        />
      </div>
    </div>
  );
}
